...
If stamina reaches 0, they must start rolling for con for every action or movement declaration that requires drawing stamina. The con roll is penalty modified by negative stamina but ignores the mod stack, etc. If the con roll succeeds the character will go into negative stamina.
...
area is clutered with large objects +3\\
...
Some knockback effects have a str value which is then used in a str vs str against the target to see if the knockback occurs.


\item[Knockdown]
The target will be knocked down and go prone unless they pass a dex roll. Some knockdown have a str value which is then used in a str vs str against the target to see if the knockdown occurs.
...
Trigger Tom just fired his crossbow at the brutal orc running towards him. No way can he reload in time. Loaded Laban has his crossbow ready but no target. They get this bright idea: LL tosses his crossbow to TT so TT can fire again quickly. What? No rule for rate of fire when tossing loaded crossbows?
