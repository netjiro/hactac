...
Great if you post your stuff as you go. Don't be shy.
...
Characters act in order of initiative. Characters with high initiative can choose to act before or after charactes with lower inititative. Characters with higher initiative can also interrupt the movement and action sequences of characters with lower initiative.
...
Attack actions are also a little different in that the attacker cannot move away after an attack before the target has taken their right to react action.
...
\action{Actions: "pick up, put down, drop"} are normal 3ap actions with a dex+9 roll. Just dropping things takes no time and requires no rolls, but carefully putting something down and grabbing an item always does.
...
\action{Action: "jump"} an obstacle or chasm is a normal 3ap action and requires a dex roll, modified by distance and speed. A mod-3 for every square the character tries to jump over beyond the first. E.g: jumping over a one square chasm is a regular dex roll mod-0, a two square burning pit is a dex-3 roll, and a 4sq creek is mod-9.
...
Better success allows for faster climbing if the Hero has enough mp to spend. Success+3 allows 2sq 2mp climb in a 3ap climb action. Success+6 3sq, +9 4sq, etc.
...
Characters regain one stamina at the start of every round.
If stamina reaches 0, they must start rolling for con for every action or movement declaration that requires drawing stamina. The roll is penalty modified by negative stamina but ignores the mod stack. If the con roll succeeds the character will go into negative stamina.
...
\todo surprise, int to get Maneuver mp and 3ap, fail int gets total surprise
...
The target must pass a dex roll or go prone, mod-3 per extra square.


\item[Knockdown]
The target will be knocked down and go prone unless they pass a dex roll. Some knockdown have a str value which is then
...
%E.g: armour takes dam/3 as well. When hp reaches 0 it reduces abs by 1, and resets hp at half hp. plate abs 3 hp 30 will be damaged plate 2 hp 15 then ruined plate 1 hp 7. \\
%? Hmm How to automate ?
%? relevant ?
...
%--------------------------------
\todo meh, rewrite excessive strength bonus points allocation structure
...
%-------------------------------------------------
There can never be rules for everything. The tiny basics cover at least 90\% of what is usually attempted during regular play. The extended rules probably 95-98\%. But there will always be edge cases. Figure out something that could work, and makes sense alongside the rest of the game.

E.g: Tossing and shooting a loaded crossbow.
Trigger Tom just fired his crossbow at the brutal orc running towards him. No way can he reload another arrow in time. Loaded Laban has already loaded his crossbow. They get the bright idea: Laban tosses his crossbow to Tom so Tom can fire again quickly. What? No rule for rate of fire when tossing loaded crossbows?

The toss - catch is almost a regular give \& receive relay action. And a quick throw attack action is 3ap as well, with mod-3. So, approximate: throwing something to a friendly target who is trying to catch it is perhaps easier than throwing at a moving target with intent to damage, so skip the quick throw mod-3. Instead of a dex+9 on either side let's say it's a dex+throw on either side to throw and catch. If either side misses it falls to the ground. A good throw is easier to catch, so let the throw success diff mod the catch action.

But both sides also have quickdraw, which covers relay, so let them roll for quickdraw to see if that will reduce the ap cost for either of them. More fun when things stack nicely.
...
%------------------------
Gold and Glory tactical gameplay requires larger maps than normal because there is often a lot more movement and positioning going on during the fights than in most games. Most battle maps I build are at least 30x30sq, sometimes upwards of 100x100sq.

Be careful when using smaller pre-made battle maps. It's best if they have firm walls or enclosure visible around the battle area, otherwise they have a tendancy to feel artificially cramped when the players want to move around. Or simply expand the map by sketching surroundings, cut'n'paste, or similar. It won't be as pretty, but allows more movement.
...
Maptool scripting is passable for very simple stuff but horrible to work with. Despite this, I've implemented automation for most everything in the game over the years. Almost all actions as GM is one or two clicks by now, and very little to keep in the noggin between rounds.
