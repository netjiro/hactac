...
\item{bolt, zap, flash:}
Bolts don't take target \emph{to hit} modifications that affect the spell casting. They ignore things like target partial cover, shields partially in the way, target movement speed, partially obstructed view by fire, etc.

\item{spear, lance, dart, arrow, quarrel, missile:}
These work similar to bow arrows. The spells take target \emph{to hit} mods directly as casting mods. Target size, target movement speed, partial cover, shields in the way, shooting into melee, fire, etc all mods the spell casting roll directly. These are the same mods as a firing arrows from a bow would take, except from 

\item{strike, spark:} % mental lock spells ignore cover etc, but mods by psy etc
