...
%----------------------
All magic skills and spells must be learned from a teacher, book, or similar. Unlike normal skills, they cannot be trained from nothing without guidance. This includes the base skills \emph{magic} and \emph{spell caster}, as well as support skills like \emph{power casting}, and all spells.
...
%\TODO It is possible to learn spells from captured spellbooks, old tomes, etc. Takes quite a bit of time. Each week, roll int modified by the spell's int requirements.

Some magic items can be used without any magic skills, others require that the user has some knowledge of the \emph{magic} skill. To learn and cast spells it is necessary to have trained \emph{spellcaster}, and that is least expensive directly when the character is created.


\openskillslist


\skill{Skill scf 2.0: "magic":} The caster can not spend more mana on each spell casting than his level in magic. This also limits the amount of mana he can pump into an already active spell each round, or use to activate or power a magical item.
...
Not all magical devices, gadgets, trinkets, and weapons require the magic skill to use. Some just require being drawn, calling an activation word, or other circumstance. Some gadgets will also draw mana from the user by themselves without the user needing the magic skill to infuse them with power. Some don't even bother with consent or intent from the user. Rare items items are sometimes powered by themselves or other sources.


%"spellcaster":} "spell caster":}
\skill{Skill cost special "spellcaster":} is required to be able to learn and cast spells. It is not required for the use of magical items nor to train the magic skill.
...
\skill{Skill scf 3.0 "drainage ritual":} Similar to blood magic, but drains mana directly from the victim. The ritual takes 5r to start then the caster must pass psy vs psy against the victim each round to drain 1+success/3 mana.
...
\skill{Skill scf x.x: \textit{spell}} Most spells have a default skill cost factor of 0.33. Some are cheaper, others more expensive. Most spells have minimum requirements on psy and int.

If the character does not meet the int requirements, the cost of the spell is doubled for each missing int point: e.g: Wizard Woolhead has int 4 and wants to learn an int 7 spell. He is missing 3 int points, thus the spell costs $2^3=8$ times as much to learn. He then spends his xp on a simpler spell.
...
Please note that all spells, unless specified otherwise, have full friendly fire effects. E.g: fear centred on self means even allies will get afraid of you, and centering fear on other means the caster also have to roll. Fireball will burn friendlies just as easily as foes. Trap embedded blasts will trigger just as quickly on forgetful friendlies as unaware unwelcomes.
...
drain          cast 5r 1m, psy/2 vs psy, range contact
               ...
               Target takes drained amount in damage, penetrating.
...
\noindent
\todo spell ideas to ruminate and evaluate
...
re-place    switch place of chars or move a char elsewhere

step-up     give extra mp to someone
            or give mp from one char to another

handover    teleport item from one to other

frighten    reduce fear resistance, or actual fear attack ?
embolden    bolster defence in general or get back on track
            after failed to resist a fear attack ?
confusion   roll for rotation and wrong attack angles
            mod-3 to actions ?

elevator    elevate or depress ground

terrain changes can be so incredibly op
wall        a quick wall of some standard material
tunnel      a quick temporary passage

shoulder view   get access to other hero's vision
                or monster's vision

alarm       when someone pass through a square or similar

noise       place a noise source somewhere else
            noise, smell, light, etc

decoy       place an illusion of a hero somewhere

mirror image   and perhaps ability to switch place with an image
               with reorientation penalty
               if int enough perhaps the images can act independently
...
minion missile cast 1r 1m, dam 4+2/m, speed 20, seeking 10 -2/r,
               ...
               Lifetime is since cast.
               Seek deterioration since targeted 10,8,6,...
               ...
               unless they miss their seek roll (once per round).
               The target does not have to be in line of sight from the caster
               once assigned. Seeking penalty only after target assignment,
               not total time since summon.
...
hold           cast 1r 2m, +1target/2m, range 10 +5/m, duration 3r +1/m
               Target cannot take movement points, but stationary actions can
...
paralyse       cast 2r 2m, +1target/2m, range 10 +5/m, duration 3r +2/m
               ...
               Continue paralyse on psy-3 vs psy +1/m each round
...
fear           cast 1r 1m, psy vs psy +2/m, radius 5 +2/m, aura from self
               +1m: touch other to center aura on target,
               self/center target is unaffected.
...
close door     cast 3ap 1m, range 15 +10/m, targets +1/m
               Close a door.
               If caster has opened the door himself he doesn't need
               line of sight if he can pass an int-3 roll.
               scf 0.2
...
dislocate      cast 2r 1m, range 20 +5/m, target 1 +2/m, psy vs psy +3/m
               ...
               the character can see at the moment.
               As long as all heroes have been successfully dislocated,
               or using individual views and strict token ownership.)
               int 5, psy 5
...
heal           cast 3r 1m, heal 3 +2/m, time 1hp/r, range contact
               int 5, psy 5
               scf 0.5
...
friendly wall  cast 2r 1m, size 3 +1/m, duration 10r +5/m
...
rot            cast 2r 1m, dam 6 +3/m 1hp/r penetrating, range touch,
...
trap           cast 3r 1m, duration 100r +100r/m, capacity 5 +5/m
               Embed a magic spell in an object and seal with a trigger rune,
               to have it activate later on.
               Trigger can be simple things like timer, touch,
               someone coming close, etc.
               Complex triggers require more mana and int rolls.
               int 8, psy 5
...
More spells on the way, still not balanced, lets try them out
...
destructive dispel
    ...
    i.e. target spell disintegrates in a blast with damage and radius
    equal to mana
...
fiery eye        int 5 psy 5
    ...
    range to target object is unlimited once cast.        eg 90deg, 45deg/r rate
                                                          of rotation)
...
pillars   -    space control   create pillars or objects /  barriers
              temporarily, or during concentration ?
...
I fully expect some people to really dislike this stuff. If so, just ignore it. It is supposed to be your game. That's the whole point here: Just use what you like, and ignore, or change, the rest.
...
Except for Quaff! most stuff is exotic equipment or custom made stuff.
...
Banors Gate    A large orb set in a stretcher-like carrying contraption.
               ...
               Touching the orb with two hands also teleports the orb gate.
...
Ear Whisper    Small worms you stick in your ear for instant communication
               between wearers up to 100sq. 1r to put in or pop out.
               All worms in one brood batch use the same "channel" that no
               other worms can hear. Just whisper and the worms will pass it
               on to your comrades.
...
Football       glow 2sq light, destructive feedback triggered range 6sq
    ...
    .           magical weapon/armour is protected, instead: blast dam6 radius 2
...
Longsword      dam 10, abs 16, str 8, max +2 str bonus, rest as long sword
...
dagger         Drains-converts hp/2(ru) to mana per round (after 10r start).
               The drained mana evaporates 1/r after target psy rounds.
...
%------------------------------
Sooner or later the powerful or rich Heroes will start fiddling towards making or buying custom magical items, weapons, armour. Probably with permanent magic, perhaps even self powered. This should be insanely expensive. For example, a sword with a permanent black bolt, even if it takes 3r to fire and draws mana from the user, is worth in total at least 100-200xp if you add up what that character would have to spend to learn spell caster, magic, black bolt. E.g: assume he's 400xp, spell caster is 60xp, magic 3 is 18xp, black bolt 9 is 24xp, assume determined and intelligent both to 3 is needed for another 18 + 18xp, trusty armour perhaps 2*5xp, and trusty sword and shield 5+5xp, for a total of 158xp.

Assume a baseline approximate money vs xp reward guideline is around 1s/xp, that means a baseline cost of the sword would be 15-20g. But then the sword can just be handed between characters, meaning it has a huge mana pool, and much higher usability than if just one character trained the skills. And on top of that the casting is probably not modified by the wielder's mod stack, and probably doesn't fail on roll of 10?

Nope, it really has to be \emph{very} expensive. How about 50-100g?
...
Perhaps it is actually just forcing a trans-planar demon to materialise in the world and through sheer will partially possess or magically command it to execute a limited set of action types. Then it will dematerialise.
