...
Teachers can be the village witch, a professor at the City Magic College, or an ally in the party who happen to know that spell. When creating a new character with magic skills and spells it's assumed that he had access to some magic training when growing up, and can thus choose quite freely from the lists.


\begin{description}


\item[Skill scf 1.0: "magic":] The caster can not spend more mana in one casting than his level in magic. This also limits the amount of mana he can pump into an already active spell each round.
...
\item[Skill scf 0.5: "power casting":] is when the spellcaster pays extra mana power to increase his chances for the casting roll. The caster gains mod+1 for each mana spent. The caster can not spend more mana than his skill points in Power Casting. This cannot be used to raise the end total chance of success above 9.

Mana spent for "power casting" is not counted to the maximum total limit set by the casters "magic" skill level.


\item[Skill cost special "fast magic":] Casting spells faster than normal. Most magic is cast in time resolution of whole rounds, for which the caster generally don't suffer action mods. But, if the Hero has to fling that black bolt a bit faster he can use fast magic and declare lots of ap to squeeze out the magic a bit quicker and perhaps manage to do something else that round as well. \\
For the purpose of casting spells faster than the time resolution of 1r, a casting time of one round is considered to be 9ap.
E.g: a 3r spell with fast magic 2 will take 2r to cast. A 1r spell will take 6ap to cast. A 2r spell will take 1r + 3ap to cast, or 0r + 12ap, if the Hero so prefers.

The caster can always choose to cast an Xap (X<9) spell in a full round instead, and take no action mod from having to declare too many ap. But then he can't do anything else in that round of course.

\small \begin{verbatim}
lvl 1, cost  15: spells cast in 75% of the time (round up).
lvl 2, cost  30: 66%
lvl 3, cost  50: 50%
lvl 4, cost  75: 40%
lvl 5, cost 125: 33%
...
\item[Maneuver cost 10: "held casting":] is used in conjunction with casting spells, delaying the final touch of the spell, so that the effect can appear at very short notice and with great control. Primarily used together with "anticipate" to inject the effects of a prepared multi-round-casting spell at precise points in the action sequence of a battle round. It costs 3ap to cast a held spell.


%TODO ? really keep efficient casting ?
...
Spells can also be deduced from magical objects and other sources, with a lot of study and some heavily modified "magic" rolls.

Consider the spells listed below to be suggestions only. Feel free to build individual spells to make the use of magic more varied and unreliable. Perhaps the Hero wants to purchase a "Black Bolt" spell, but the grey old master only teaches him the "Thread of Dread Darkness" which is very similar to the "Black Bolt" but with some minor changes in range, effect, etc.
...
\item[Skill scf x.x: \emph{spell}] Most spells have a skill cost factor of 0.33. Some spells are cheaper, some more expensive. Some spells have minimum requirements on psy or int. If the character does not meet the int requirements, then the cost of the spell is doubled for each missing int point: e.g: Wizard Woolhead has int 4 and wants to learn an int 7 spell. He is missing 3 int points, thus the spell costs $2^3=8$ times as much to learn. He then spends his xp on a simpler spell.
...
Spells are trained as regular skills and have scf 0.33 if not specified otherwise.

Spells with more complexity have higher int requirements, and spells with higher inherent control power has higher psy requirements.
...
shock bolt     cast 2r 1m, dam 7 +2/m, penetrating,
               range 20 +10/m, long -3dam,
               int 7, psy 7

shock bolt     cast 1r 1m, dam 5 +1/m, penetrating,
               range 15 +10/m, long -3dam,
               int 5, psy 5

force bolt     cast 3ap 1m, dam 5, penetrating 1,
               range 10, long -2dam,
               int 7, psy 5

force bolt     cast 3ap 1m, dam 6 +1/m, penetrating 2,
...
hasten         cast 2r 2m, +1target/2m, range touch, duration 3r +1/m
               Adds 3ap +1/m to target each round for the duration.
               Initiative is raised by 3+3/m.
               The character also gets 50\% extra movement points each round
               (round up).
               int 8, psy 8
...
pilfer         cast 2r 1m, range 5 +5/m,
               Can steal one item that is not held from the target.
...
heal           cast 3r 1m, heal 5 +3/m, time 2hp/r, range contact
               int 7, psy 7
...
raise dead     cast 5r 2m, duration 3*diff +10r +10r/m, range touch
...
mass raise     cast 10r 3m, duration 3*diff +10r +20/m, range 5 +5/m
               target diff +1/m
...
drain          cast 2r 1m, psy vs psy +3/m, range contact,
               ...
               Roll psy once per round to wake up again.
               int 5, psy 8

darkvision     cast 2r 1m, vision 15+5/m arc 180+90/m, duration 20r+20r/m
               int 3, psy 2

rot            cast 2r 1m, dam 4 +1/m 1hp/rnd penetrating, range 8 +2/m,
...
revive         cast 10r 5m, psy vs psy +1/m, the dead wanna stay dead.
...
fumbly         cast 2r 1m, duration 5r +2/m, range 5 +2/m, target 1 +1/m
               ...
               Not action stack, thus ignoring quick etc.
               int 4, psy 4
...
stun           cast 6ap 1m, range 10 +3/m, target 1 +1/m
               Stun 9 +6/m
               int 5, psy 5
...
big boss' eye  cast 2r 1m, duration 10r +5/m, move 5sq/r +3/m,
               ...
               high in the air. The eye cannot pass through solid (1+sq) walls,
...
sacrificial    dam 1, penetrating 1, ritual sacrifice 10r,
dagger         drains-converts 1hp to 1m /r,
               until the target dies from hp or mana loss.
...
energy rod     holds 10 mana, transfer 1-10m/r out, transfer 1m/3r in
...
psycharmour    abs 2 +2/m must declare and roll for skill before damage is dealt
               ...
               Defensive mana spending is not an action, it's an interrupt.
               Holds 15 mana, transfer 1m/2r in/out.
               Transfer must be primary action each round.
               psycharmour skill is scf 0.33

powerstaff     dam 3/4 (1h/2h), abs 10, str 3 (max +1 str bonus)
               Holds 20m. Can only release 1 point per spell casting, and the
               released mana must be used to reduce the spell mana cost to the
               caster by 1 point. Release is a 0a free action, included in the
               spell casting.
               Cannot transfer power from staff, but can transfer mana to staff.
               Transfer to staff 1m/r, 1a primary action.
...
Long Sword of  dam 8, abs 14, str 6
Irdereng       on successful attack the attacker can choose to roll psy,
...
(heavy)        same rules for unconsciousness as "casting when dry".
               ...
               purposes of damaging magical and non magical creatures.
...
Helmet         It will draw one mana at the start of every 100 rounds.
...
PortaPortal    Two large metal discs, comes as matched pairs. When both are
               ...
               for about 10r. A disks must not be moved once the seal has
...
weapon +x     A weapon that is easier to use or does more damage. It can give
              ...
              to both.
              Weapons like this does not have to be magical. +1 and even +2
              weapons can manufactured by master or legendary craftsmen.
