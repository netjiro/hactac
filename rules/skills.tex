...
Regular skills and maneuvers can be bought by XP and trained from scratch without a teacher and prior knowledge. \\
...
\section*{Variation, change, force players to adapt and think}
%-------------------------------------------------------------
I intend to never lock down the cost of skills hard. Instead I'm developing approximate ranges, and hopefully one day will have the time to write up a small script that will assign random cost figures for each new campaign, within a set range of "reasonably interesting" values.
...
If SSS had his compadre Permanently-Planted-Pete by his side, also coming into weapons reach when MM moves in, then the first attack from PPP as response to the movement would also have the engage mods from MM.
If the incoming attack is a combination attack action, e.g. double, then both the attack rolls take the penaly, because they are done in the same action.
...
\closeskillslist
\todo rewrite block/tackle for 0ap and 3ap active/inactive resistance, offensive must take 3ap action
\openskillslist
...
\skill{Skill scf 1.0 "mind games":} The clever death dealer can spend time and put intellect to good use finding weak aspects of their opponents' reactions.
...
\skill{Skill cost 1.0 "make it count":} Can increase damage of a hit.
Declare to make the next attack count, and how, before rolling the attack. A declared make it count attack must be made regardless of the outcome of the make it count roll. A successful roll will add the success diff as damage to the weapon damage, before the final damage roll.

A make it count attempt costs 1 stamina above the usual attack. The Hero can choose to either spend +1 ap extra \emph{or} risk getting a more difficult attack. This must be decided before rolling. Without spending extra ap, a failed make it count roll will pass the fail diff as negative mods to the attack roll. Spending the +1 ap extra before the roll removes that possible negative mod.
...
Any successful attack on a roll of "1" can do extra damage if the hero then passes a critical damage roll. After rolling the first "1" for the attack, the Hero can declare a critical attempt, pay 1 stamina, and roll for critical damage.

Depending on the success level of the critical damage roll the total damage is increased:\\
fail : it's just normal damage, rolled as usual, no critical damage \\
success+0 : total damage is rolled +50\% of max, min +3 \\
good success+3 : total damage is rolled +100\% of max, min +6 \\
very good success+6 : total damage is rolled +200\% of max, min +9 \\
perfect success+9 : total damage is rolled +300\% of max, min +12
...
A successful quickdraw roll on an item in a quickdraw slot means the draw action is an instant 0ap action instead of a normal 3ap action. Very useful for quick changing weapons or drawing new arrows and throwing knives.
Spending 3 quickdraw "slot point" on one item means it will always be a 0ap instant action to draw and stow without a roll.
...
Navigating the region map requires successful track rolls. Following roads is mod+6, plais mod+3, moors=0, hills and forests mod-3, mountains and deep forests mod-6. Weather and time of day or night can also play a big role.

Having a map gives mods as long as the character has int enough or literate. Most maps give positive mods but very poor maps, or inentionally misleading maps, can give negative mods. Equipment like compass, spyglass, etc also helps. Histography can also help with navigation if there are relevant landmarks or sites in the area.
...
The histography skill also aids in navigating the region as long as there are any relevant landmarks or important sites: of historical, religious, social, or economical relevance. If so, apply 1/3rd or 2/3rd of histography as mods to the track roll.
...
\skill{Maneuver (cost 50) "assault step":} allows for the character to move one extra sq beyond, declared movement, together with, but before, an attack action. This does not force the character to go off balance, but it costs two stamina.
...
\skill{Maneuver (cost 10) "draw-fire":} when using a "heavy" bow the shooter can skip the usual str+2 requirement if shooting with the draw-fire attack maneuver.
A draw-fire costs +1r above the normal attack and costs +1stam above the normal attack. It cannot be aimed or combined with held fire. The extra +1r time cannot be shortened by other maneuvers and skills.

E.g: Robin Wood can shoot in 1r at mod-0 with quick shot 3. He can make a draw-fire attack with a heavy large longbow in 2r. He can do so at str 7 even if the heavy large longbow has str 9 requirement.
...
It's a regular attack which costs 1 extra stamina and has mod-3. If successful and not avoided the attacker and target will take a str or dex based resistance roll to determine if the target falls down or not. The attacker takes the knockdown attack success diff as mod to his side of the resistance roll, and the target takes his defence diff (positive or negative) to his side of the resistance roll. Target can apply balance to his resistance roll.
...
\skill{Ability cost 30xp ccf 1.1 "Foul":} The character is the last target, among other equally accessible targets, for monsters lacking int. GM discretion.
...
\skill{Ability cost 30xp ccf 1.1 "Bland":}  The character is the last target, among equally accessible targets, for monsters who fail their int roll when seeing the character for the first time. GM discretion.
...
cost +20xp: disruptive: mod-3 to all spell casting in psy/3 range.
