...
\subsection*{Cost of improving skills}
%-------------------------------------
...
A noob Hero starts with at most \verb|xp/10 + int/3| skills for free, without having to pay to get them on the list.
...
It could be fun to require that a Hero first must have observed someone use a fighting technique / maneuver / skill before he is allowed to try to figure it out by himself. Or perhaps otherwise take a mod-3 to the int roll?
...
\skill{Skill scf 4.0 "weapon master":} is the dude who knows how to handle just about any weapon imaginable, including obscure varieties of unarmed combat.
...
Brawling also includes grab, hold, break loose actions. They are resistance rolls where both participant uses brawl + the lowest of their str and dex, and costs 3ap for both participants.     %previous wrestle
...
\skill{Skill scf 1.3 "martial arts":} is 80's tv trope Asian inspired unarmed combat. A character generally has two fists and two kicks to use. This can be combined with the skills "quadruple", "triple", "double". \\
...
%\TODO:
%avoid is faster for tiny and smaller stuff:
% 3ap: medium, small     (human,orc,goblin, dog, )
% 2ap: tiny,             (runt, cat, large bat, large spider, large bug)
% 1ap: pytte             (sprite, fly, humming-bird, swift, ...)
%slower for larger monsters
% 2x2: 4ap
% 3x3: 5ap
% 4x4: 6ap
%but can still be fast for tentacles, legs, tails, etc for the large monsters
...
\skill{Skill scf 0.5 "engage":} allows for the character to make a safer movement \emph{into} opponent's weapon reach modifying the incoming attacks. The level of engage directly modifies the opponent's attack against the engaging target.
Just like disengage, engage is also not an action and "always on".

It's only the first incoming attack directly triggered or declared in response to the movement that is modified, for each attacker.
...
\skill{Skill scf 1.0 "mind games":} The clever death dealer can spend time and put intellect to good use finding weaknesses in their opponents' fighting behaviour.

\todo rewrite mind games, this old version is broken
% why spend 1r for one attack, when the opponent can spend a second action to attack the hero who now has 0ap left for defence ?
...
The doppel cap is also modified by ams as though it was a roll. E.g: doppel 11 mod-4 is doppel cap 7.

The doppel cap can be focus shifted between the two combined actions. Taking a mod-1 on one of the actions gives a mod+1 to the other. But the mods cannot raise the chances higher than the action + ams would be without the doppel cap. E.g: Shield 10 sword 9 doppel 8 ams 3 is effective shield 5 sword 5 for a combined action, and balance shifting cannot raise to more than shield 7 or sword 6.

The actions combined within doppel, can also be focus shifted like normal actions.


\skill{Skill scf 1.5, 2.0, 2.5, ... "trippel, quippel, peppel, etc":} similar to "triple, quadruple, etc", but based on the "doppel" base functionality.


\skill{Skill scf 1.0 "whirlwind":} The character can make several fast attacks within an extended whirlwind chain action. The first attack is done at normal speed for the weapon but each following attack is fast 2 (min 1ap).

Before each attack in a whirlwind sequence the character must pay one extra stamina and roll a successful whirlwind check. When the whirlwind roll fails no more attacks can be done in that whirlwind sequence. The declared attack is then done as a normal attack.
...
Whirlwind becomes even more dangerous when combined with for example double. Then each weapon has its own the damage cap track. It can also be combined with different attack maneuvers but all attacks in the sequence will use the same type of attack. \textit{Or} keep track of damage cap as percentage of max and allow different types of attacks capped by the lowest percentage thus far in the sequence.
...
A high initiative character can Oy!, or use a regular communication action, to activate a lower initiative ally so they can coordinate a synchronised attack. This is often done by a third party "commander" to get a combat unit to synchronise their attacks.
...
The final damage done by the backstab and weapon attack is based on the success of the weapon attack roll. Add the backstab bonus damage to the weapon damage and resolve from there like a normal attack. E.g: Backstab with a shortsword, a backstab roll success+4 gives dam+6 pen+2, added to the regular shortsword weapon damage of 5 gives a total max damage 10 penetrating 2. Assume the attacker has consistent 2 and rolled a weapon attack success+5 after the backstab success+4, then the final damage will be rolled dam[4,11] pen2.
...
\skill{Skill scf 1.0 "sidestab":} allow the attacker to time his strike against the target to coincide with an ally's attack, increasing damage. A sidestab costs +1ap and +1 stamina extra above the normal attack, \emph{or}, the sidestabber can take an extra mod-6 to the sidestab and attack rolls instead of paying the extra +1ap.
...
%\TODO: NOTE 200102: the sidestab can be performed multiple times against an aware target, not just once like backstab. Thus sidestab is set at cost scf 1.0 instead of 0.5 like backstab. Sidestab is situational on coordination whereas backstab relies on sneaking and positioning.

%\TODO: Perhaps reduce potency of sidestab?
...
\skill{Skill cost 1.0 "make it count":} can increase damage of an attack. A make it count attack is first a roll against the make it count skill followed by a weapon attack roll. All in one action. A successful make it count roll will add the success diff to the max damage of the weapon attack roll. A make it count attempt costs 1 stamina above the usual attack.
...
\skill{Skill cost 0.5 "critical damage":} The Hero has a small chance of doing much higher damage on some attack hits, if the dice favour him.
...
\phantomsection \label{skill:tank}
...
lvl 6-9 cost+15 each: chain, plate, full, heavy: as two classes lighter \\ % =110
lvl 10-12 cost+20 each: plate, full, heavy: as three classes lighter \\    % =170
lvl 13-14 cost+40 each: full, heavy: as four classes lighter \\            % =250
lvl 15 cost+100: heavy plate as five classes lighter, i.e. no mods         % =350

\noindent E.g:\\
lvl  2: Chain mail as leather: 20xp\\     % per heavier step to same mods:
lvl  7: Plate as leather: 80xp\\          % diff + 60
lvl 11: Full plate as leather: 150xp\\    % diff + 70
lvl 14: Heavy plate as leather: 250xp     % diff +100

%  1-5 +10         -1         +50 for hvy =0 to -1         50
%  6 +15  65 chain -2
%  7 +15  80 plate -2
%  8 +15  95 full  -2
%  9 +15 110 hvy   -2         +60 for hvy -1 to -2        110
%
% 10 +20 130 plate -3
% 11 +20 150 full  -3
% 12 +20 170 hvy   -3         +60 for hvy -2 to -3        170
%
% 13 +40 210 full  -4
% 14 +40 250 hvy   -4         +80 for hvy -3 to -4        250
%
% 15+100 350 hvy   -5        +100 for hvy -4 to -5        350
...
A trained jumper can make a jump in a 3ap action instead of a full round action. It's also possible to use fewer ap, taking a mod-1 for each reduced ap. E.g: a 0ap jump is mod-3.
...
Quickdraw also works for holstering and stowing items, picking up or placing items, give \& receive handovers, relay, and similar. %relay handover
...
To have faster give \& receive handovers the Hero can dedicate a quickdraw slot for "give \& receive" to make it a non-roll 3ap action. %relay handover
...
\skill{Skill scf 1.0 "blindfight":} reduces any action mods from being blinded by the level of blindfight. Also for actions which are not usual "fight" actions.
...
\skill{Skill scf 0.5 "track":} is finding stuff in the wilderness or follow people in dirty caves or other places where trace of passage can be left behind. It is used for following a trail for example, and to forage for food when the provisions run out. Track also sets the vision radius of the party when travelling on regional maps.
...
\skill{Skill scf 0.5 "learn":} makes it easier for the Hero to learn things by himself. Add the learn level as mod to int rolls when trying to figure out new skills, improving skills, or learning from a teacher.


\skill{Skill scf 1.0 "teach":} makes it easier for students to learn new skills. Each level of teach gives +1 to the students' int rolls to learn a new skill the teacher already knows.
...
A character with companion command can control as many companions as his skill level/3 (ru). Companions can be trained as combat units, so each unit counts as one companion when you calculating command control numbers.
...
\skill{Skill scf 0.5 "meditate":} Sit down and concentrate to rest faster. Roll once per round meditating. Regain +1+success/3 stamina above normal rest.
E.g: meditate success+5 regains +2 stamina above the normal resting stamina recovery. Success=0 rest+1, success=3 rest+2.

Meditating in a combat zone is mod-3. Need "calm" in range 10-psy.
During meditation the Hero has per-6.


\skill{Skill scf 1.0 "tunnelist":} This Heroic digger can carve out tunnels and hack through objects at amazing speed. Each level of "tunnelist" increases digging/hacking speed by 50\% of base speed.
E.g: A tunnelist 5 is hacking away at 3x the normal speed.
...
%\TODO: go through battles and see when quick 3 mobile 3 rapid 3 are no longer
%       obvious nobrainers
% still too cheap:
% quick  4.0   parity with raising several skills +1-3 around 8-12?
%              depending on how many actions per round taken avg
% mobile 3.0   parity with raising fast enough to get higher move
%              mobile around 3  vs walk speed ?
% rapid  1.5   how important is the elevated initiative ?
%              in practice across fights


% NOPE:  quick should be more expensive than just raising the main weapon lvl+3
%        ? and shield (0.7) ?
% sword 11 - 8 : 57xp
% shield 11 - 8 : (8 - 5) with regular shield parry+3:  39*0.7 = 27
% so quick 3 should cost more than 84xp  :    90 xp  ?
% quick lvl 1  30            17 +  7 = 24  :  25 xp
%           2  50            36 + 16 = 52  :  55 xp
%           3 100            57 + 27 = 84  :  90 xp
% just sword
%                            17               20
%                            36               40
%                            57               60   sword+shield / sword only
% +6ap  14 - 8 / (11 - 5)   132 +  67 = 199          200           140
% +4    12 - 8 / ( 9 - 5)    80 +  39 = 119          120            80
% +5    13 - 8 / (10 - 5)   105 +  52 = 154          160           110
% +6    14 - 8 / (11 - 5)   132 +  67 = 199          200           140


%\TODO: perhaps set quick cost as a list,
%       10, 25, 45, 70, 100, 135                 +10,+15,+20,+25,+30,+35
%       15, 35, 60, 90, 125, 165                 +15,+20,+25,+30,+35,+40
%       (10 * lvl) +  (4 * lvl^2):
%       14, 36, 66, 104, 150, 204                +14,+22,+30,+38,+46,+54
\skill{Skill scf 6.0 "quick":} gives one extra action point per level. \\
...
\noindent \todo fix quick cost


\skill{Skill scf 4.0 "mobile":} reduces the movement penalties from declaring faster movement by one for each skill level. \\
E.g: mobile=2 means maneuver mod-0, walk mod-1, run mod-4, dash mod-7.


\skill{Skill scf 2.0 "rapid":} Improves initiative by one for each skill level.


\skill{Skill scf 2.0 "veteran":} reduces fear, morale, and pain penalties by one for each skill level. Also gives a bonus to psy for rolls against fear and morale equal to level.


\skill{Skill scf 2.0 "tireless":} reduces mods to actions due to negative stamina by one for each level.
...
The character must pre-pack his luggage with \emph{useful bundles} that take up space and weight until they are unwrapped, revealing useful items. Prescient packing cannot be used when there are not enough useful bundles left in the luggage to cover the encumbrance value of the sought item.
...
\skill{Maneuver (cost 15) "opportunity":}
...
%E.g: Sebastian Slouch has yield+2 bonus to start with. He curses his genes, spends 15xp (30-15=15), and gets yield+3.
...
When jumping, dive allows to tuck +1sq to jump distance, if landing prone.
Dive does not cause the character to go off balance, just prone.
...
\skill{Maneuver (cost 30) "defensive step":} allows for the character to move one extra sq, beyond declared movement, if yielding or dodging together with a defence action.
This does not force the character to go off balance, but it costs one stamina.


\skill{Maneuver (cost 50) "assault step":} allows for the character to move one extra sq beyond, declared movement, together with, but before, an attack action. This does not force the character to go off balance, but it costs two stamina. It can only be performed if the step will take the Hero into weapon reach where he will make an immediate attack.


\skill{Maneuver (cost 50) "step away":} like assault step, but movement must be taken after the attack action, though still together with it.


%TODO: follow wake   fix cost, fix wording
\skill{Maneuver (cost 30xp) "follow step":} allows the Hero to take an extra step beyond declared movement if following in the wake of a target moving out of melee. The follow step action is triggered immediately when the target moves away, ignoring initiative, but does not allow an immediate attack. Compare with opportunity.\\
\todo: follow wake   fix cost, fix wording


\skill{Maneuver (cost 15) "run away reserve":} allows the hero to declare run or dash speeds even if he has below 33\% hp, and can run but not dash if below 0 hp. He must declare 0ap and run away from the opposition, GM discretion. He does not need to roll for stamina even if he has negative stamina to begin with, as long as his stamina is not below -con. The stamina cost for the movement has to be paid as normal.

The Hero must pass a psy roll to stop running away. This can easily carry the Hero off the map since it is usually declared when the Hero is in poor conditions with lots of mods from wounds etc.
...
\skill{Maneuver (cost special) "fast shield":} allows the shield bearer to make shield defence actions at 2ap instead of 3ap. Available in 3 levels:\\
...
\skill{Maneuver (cost 20) "careful":} allows the character to make an action or movement with a mod+1 to succeed, and where the "10" roll is not an automatic failure. Being careful takes longer time.
...
Parries and deflection defence actions get a mod+1 for each of the (two possible) sides that the Hero has covered by friendlies who also use phalanx. Phalanx does not provide a bonus for avoid defence actions, or if yielding with the defensive action.
...
E.g: a three abreast shield line where all users know and use the phalanx will give a parry mod+1 bonus to the edge fighters, and a parry mod+2 bonus to the centre fighter. If the edge fighters have walls on their "open sides", then they also get the full mod+2 defence bonus from phalanx.

Phalanx does not require shields, but the phalanx bonus is only available to parries, deflect, etc, stationary weapon defence actions, not to avoids.
It is not possible to maintain phalanx bonus if yielding. A phalanx must move together or loose its bonus. Therefore the phalanx usually moves in the lowest initiative of all participants, and cannot yield. A commander or tactician can use the leader or tactics skills to give the phalanx orders to move or act in their initiative.
...
For ranged attacks it allows the use of double rank firing lines, where the front line shoots from a kneeling position, and the back line fires above them. It is also possible to shoot over a shield line or standing line, but with mod-3. Thus it is possible to have three rows of archers: sitting, standing, behind (mod-3).


\skill{Maneuver (cost 10) "combat unit":} allows for all people within the same combat unit to count as one character for the sake of activation, ordering, strategy bonus (when fighting the same strategy covered target), etc. Bonused as one only applies when they are fighting tightly together and is at GM discretion.
...
\skill{Maneuver (cost 10) "draw-fire":} when using a too strong bow the shooter can take one less mod point from missing strength by using the draw-fire technique and spending +1stam above the normal attack. It cannot be aimed.
...
\skill{Maneuver (cost 5) "unfriendly fire":} When shooting into melee, unfriendly fire increases the fail gap before the shooter hits an ally in base contact with the target.

upgrade:\\
cost 5: increase the fail gap by 3 before hitting an ally \\
cost 10: increase by 6 \\
cost 15: increase by 9
...
\skill{Maneuver (cost 10) "corpse kick":} is useful for moving corpses. Spending a 3ap action the Hero can kick 1 corpse 1 sq in any direction. Large corpses (2x2) require 1r to kick and str 10+, and even larger are not kick-able unless the kicker is at least almost as large as the corpse.
...
\skill{Maneuver (cost 5) "curve runner":} The Hero can calculate \hyperref[approach]{speed of approach} through mild turns. Every 3sq he can make one 45deg turn and still count the distance before the turn as speed of approach.
...
\skill{Maneuver (cost 5) "swing":} allows for a great slow swing attack which is available with some weapons. It usually has a slow 1 mod-1 dam+2 toavoid+2 effect, but depends on the weapon.
...
\skill{Maneuver (cost 10) "fjutt":} is a fast but weak and flimsy attack. It can be performed with any melee weapon unless otherwise specified. The standard stats are:\\
fast 1 (minimum 1ap) mod-3 with damage 50\% of normal.
%TODO: skip? : It costs one extra stamina.
...
%\todo too powerful compared to roundabout. check and balance both with wayward
...
\skill{Maneuver (cost 10) "dual parry":} requires two weapons and gives a mod+2 to the parry at the cost of 1 stamina extra and 1ap extra. As baseline, choose the ap cost and parry chance of one of the two weapons. Then add half of the other weapon's abs when calculating weapon breakage against incoming damage.

\noindent Dual parry cannot be used with shields.


\skill{Maneuver (cost 10) "heavy strike":} costs +2ap for normal or slower weapons and +1ap for fast or vfast weapons, and is mod-2, toavoid+1, +33\% dam. Takes one extra stamina.


\skill{Maneuver (cost 10) "massive strike":} costs +3ap for normal or slower weapons and +2ap for fast or vfast weapons, and is mod-3, toavoid+2, +50\% dam. Takes one extra stamina.


%\todo meh, don't like mixing in the extra movement
...
%
%The followup attack costs two extra stamina but costs no movement points, even if it's a second diagonal and would normally cost 2mp. Followup attack can only be used once per round.
%
%Weapons with reach has an extra mod+1 when used for a followup attack, regardless of whether reach is used for the attack.


\skill{Maneuver (cost 10) "counter attack":} The user can parry and make a counter attack in the same action using the "counter attack" maneuver. Both the parry and attack rolls are at mod-3. The attack is dam 75\%.
...
\skill{Maneuver (cost 30) "riposte":} The Hero can make something similar to a counter attack but using only one weapon, at a cost of +1ap above the slowest of the defence / attack, and one extra stamina above the usual cost, taking mod-4 to both the parry and attack rolls, and the attack is dam 75\%. E.g: riposte with a sword is 4ap, and with a rapier it's 3ap.


\skill{Maneuver (cost 5) "strength bonus":} The Strong Hero can use extra strength, above the weapon requirement, to do extra damage or penetration depending on the weapon stats. For each +3 excessive strength he gets damage +1, or penetrating +1, depending on, and limited by, the weapon.
...
%\TODO: ? build / keep this ?
%\skill{Maneuver (cost 5) "dexterity bonus":} like strength for wepons, armour, situation, etc


\skill{Maneuver (cost special) "slugger":} gives extra damage with weapons that have extra strength bonuses. Each level of slugger gives excessive strength damage bonus counted as (extra strength)/1 instead of (extra strength)/3 as per "strength bonus". Limited to the maximum of the weapon.

\todo - sort out slugger damage bonus to brawl and martial arts relative to speed
\tmpsepline
Slugger also gives damage bonus to brawl and martial arts attacks equal to level of slugger for 3ap attacks, slugger/2 for 2ap attacks, and slugger/4 for 1ap attacks.
\tmpsepline
Slugger also gives damage bonuses to brawl equal to the level of slugger, without requiring extra str points. It also gives damage bonus to martial arts equal to slugger/2 without extra str.
\tmpsepline
%\\
\noindent
lvl  1  costs  10 \\
...
Tiny characters are limited to slugger 1, small to slugger 3, normal to 6, large to 9, and so on.

Same extra strength points do not give both strength bonus and slugger bonus. E.g: wielding an axe (str 4) with strength 7, strength bonus and slugger 2, does not give dam+3. It gives dam+2 from slugger together with two of the three points of excessive strength. Without slugger it would be dam+1 from the strength bonus and the three points of excessive strength.
...
%\TODO: fix !!   -- 210106: partial: stronger limit for small creatures
% for a high power halfling: str9 dex12 martial arts and max slugger would give damage 13 fist and 15 kick. Assuming strong 3 agile 3 slugger 6 (18+18+100=136xp)
...
\skill{Maneuver (cost 20) "fast strength":} A character with enough strength and dexterity can choose to trade excessive strength damage bonus to a fast 1 modifier for the actions with the weapon instead. Also comes with a mod-1 difficulty though. \\
...
\skill{Maneuver (cost spc) "stronk tank":}
...
\skill{Maneuver (cost 5xp) "scythian technique":} where the archer or crossbowman can shoot with their shield still hanging passive on their arm, taking mods to their bow / crossbow attack rolls. The warrior can switch between shield at ready and bow / crossbow at ready in a 0ap action. While the shield is hanging passive the archer gains no advantage from the shield being in the way. Once the shield is switched at ready the full parry mods, in the way mods, and possible hiding behind mods are available.
...
\skill{Maneuver (cost 5xp) "reithian technique":} where the archer or crossbowman can shoot while still holding a 1h weapon passive in one hand, taking mods the their bow / crossbow attack rolls. The warrior can switch between the bow / crossbow and other held weapon in a 0ap action.
...
\skill{Maneuver (cost 10) "arrow stab":} Instead of shooting your opponent the Hero can use the projectile to stab the target. Damage is 50\% and penetrating 0. The attack is mod-3 to hit, and a regular 3ap action. It counts as a regular attack for sake of stamina. For the attack skill roll the character can use the highest of either: bow, crossbow, throw, knife, rapier, sword, spear, brawl, martial arts skills.
It works just as well with rocks, chairs, bottles, and other projectiles as it does with arrows, using the brawl skill, as improvised weapons.

This move was made popular by the heroic dancer Leggy-Lars who used it in the movie "Lard of the Sphinx" to stab an orc in the throat before drawing the arrow and shooting the next orc inzahead. Since the damage is low it is suggested that it is combined with a "critical damage" maneuver.
...
%\todo ? remove breaker, too powerful?
%      or change to roll flat dX for breaker extra damage
%      or have breaker add +lvl to chance of breaking when already
%      exceeding abs of parrying weapon/shield
...
\skill{Maneuver (cost 5) "mounting jump":} Run up to your horse and do a successful jump to mount in 3ap instead of 1r.
...
\skill{Ability cost 30xp ccf 1.1 "Combat Advantage":} When the character makes a successful attack which the opponent parries or avoids he will get the "combat advantage". The advantage kicks in after the completion of the actions taken by the character and the target, and only matters between those two combatants.
...
\skill{Ability cost 10xp ccf 1.1 "Hardened":} Increases pain threshold by one for each level. Each level costs 10xp.


\skill{Ability cost 30xp ccf 1.1 "Painless":} Character is immune to pain modifications.


\skill{Ability cost 20xp ccf 1.1 "Tasty":} The character is the obvious target, among other equally accessible targets, for monsters lacking int. GM discretion.


\skill{Ability cost 30xp ccf 1.1 "Foul":} The character is the last target, among those accessible, for monsters lacking int. GM discretion.


\skill{Ability cost 20xp ccf 1.1 "Annoying":} The character is the obvious target, among equally accessible targets, for monsters who fail their int roll when seeing the character for the first time. GM discretion.
...
\skill{Ability cost 30xp ccf 1.1 "Bland":}  The character is the last target, among those accessible, for intelligent opponents who fail their int roll when choosing new targets to attack. GM discretion.
...
\skill{Ability cost 20xp ccf 1.1 "Angry":} The character gets angry when wounded. Each active pain point mod gives him dam+1 in melee attacks and str+1 to strength tests (not damage bonus). Pain that is countered by pain killers, painless, etc is not active pain. Pain that is countered by veteran is still active pain.


\skill{Ability cost 20xp ccf 1.1 "Furious Grief":} The Hero really likes his friends and gets furious when they are killed.\\
...
\skill{Ability cost 20xp ccf 1.1 "Divine Retribution":} The character has a vengeful god on his side. When he dies his dying corpus will channel the vengeance of his angry god in form of a divine blast that gives penetrating damage to everyone within range 3. Damage equals his total XP/50. The corpse is mod+3 to all kinds of revive after the divine blast, thanks to the remaining empowerment of the god's energy. The damage roll should be based on a success+9.
...
%\TODO: or should the damage apply once per round at the initiative of the character? Or an active instant 0ap action to activate the aura pulse?
% ? which version is more interesting ?
% ? should it have friendly fire as well, if manually activated ?
% At end of round: it's better if the char has low initiative, to move in and park for damage after everyone else has already spent their movement.
...
\skill{Ability cost 20xp ccf 1.1 "Breathless":} The character does not have to breathe. He is thus immune to most gas attacks, drowning, etc.
...
Dissolving the ghost takes a full round, but requires no actions, and it must be in an empty adjacent space. Dissolving it in a 3ap action gives the Hero 1hp damage, in a 1ap action 1d3 damage, and in a 0ap action 3+1d3 damage.
...
Teleporting normally takes a full round, but can be done faster. Attempting to teleport in a normal 3ap action is mod-3, in 2ap is mod-6, and in 1ap is mod-9. These mods also affect the max range psy calculation and the accuracy int and acclimatisation per rolls.
%\todo ? should it really affect the int and per rolls? more fun if not?
...
\skill{Ability cost 30xp ccf 1.1 "Lean":} Character can trace vision and line of sight from any non occupied surrounding square. Also applies when making magic or ranged attacks, but not regular melee attacks, not even with reach. Facing direction is maintained.
...
\skill{Ability cost 20xp ccf 1.1 "Tingly magic":} The character will get a tingling feeling when he is subjected to magic or targeted by the buildup of a magic spell. He will not detect clean dormant or inactive magic. He will detect leaking dormant / inactive magic, but only if he is in the spell radius.
...
\skill{Ability cost 30xp ccf 1.1 "Crowdfighter":} When fighting in a crowd, the character gets upset and gets dam+1 per opponent in base contact, beyond the first one.
...
\skill{Ability cost 20xp ccf 1.3 "Blink":} The character can blink out of existence for a few rounds, then re-appear in the same place, or closest unoccupied space. It takes one round to activate, costing 3ap, and the character blinks out at the end of the round.
...
\skill{Ability cost 20xp ccf 1.1 "NullSkull":} When rolling psy vs psy rolls against incoming magic influences the character can add +6 to his psy to resist. Mental Lock spells cannot target a NullSkull.
...
\skill{Ability cost 100xp ccf 1.1 "Über":} The True Hero is sooo cool that a roll of 10 is no longer an automatic fail. A natural 10 roll is just like any other roll of the die; if his chance to succeed is 10 or greater, he will succeed.


\skill{Ability cost 100xp ccf 1.1 "Ünder":} The True Hero is so lucky that a roll of 1 is an automatic success=0 instead of a fail if he had 0 or lower chance of success.


\skill{Ability cost 50xp ccf 1.3 "Extra Extremity":} The character grows another arm. Can be bought multiple times. A fourth arm? How about a tentacle? Perhaps you'd prefer a third leg, or some other extra appendage? People will probably look at you really strangely from now on...


\skill{Ability cost 20xp + scf 0.5 ccf 1.1 "Switcheroo":} The character can change target of an attack after the attack and damage rolls have been made, but before the target spends his reaction action. Both the originally declared target and the switched to target have the right to react to the attack.
...
\skill{Ability cost 20xp ccf 1.1 "Reactive movement":} The character can always declare movement last, ignoring initiative. When two characters both have reactive movement it is highest total xp that decides who declares last.
...
% aka "bottled fury" "furious" , word play from "angry" ?
% "wounded fury"  /  "wounded determination" ?


\skill{Ability cost 50xp ccf 1.3 "Wall Pass":} Signed by some obscure forgotten deity. The Hero is allowed to move through one square of solid materia, but is not allowed to stop inside. Costs 1 ap, 1 stamina, 1 mana for each square of solid materia passed through.
...
\skill{Ability cost 20 ccf 1.1 "Rolling Gut":} The Hero has a weird supernatural sixth sense gut feeling of the rolling destiny unfolding around him.
...
\skill{Ability cost $\geq$20xp ccf 1.2 "TronVol":} Heroes can meld together to form one large super critter. All Heroes in the TronVol merge must have the TronVol ability, and each TronVol merge target is a separate TronVol ability but does not incur extra ccf beyond the first.
...
\skill{Ability cost 10xp + scf 4.0, ccf 1.2 "gut feeling":} Before every new session/day/dungeon the character can roll and store a number of rolls equal to his level of gut feeling. These can then be used instead of making a new roll when one is called for. Each stored roll can only be used once. \emph{Storing} a roll costs no xp, but \emph{using} a gut feeling stored roll costs 1xp. Gut feeling is an instantaneous 0ap interrupt effect.
...
%\TODO this is actually the normal behaviour, just that most people don't think about it
%\skill{Ability cost 30xp ccf 1.2 "slip away":} allows the Hero to start or continue a movement immediately after an action, ignoring initiative. The target of the attack or action will still get the reaction before the Hero can move away.
...
\skill{Ability cost 20xp ccf 1.2 "escapeaid":} costs 5xp to activate! Once declared the Hero have mod+3 to all defensive actions and m+1 w+2 r+3 d+4 until they leave the edge of the map. During this period all attacks, and any other action that is not to the immediate benefit of getting \emph{that Hero} escaping off of the map, have mod-6.


\skill{Ability cost 30xp ccf 1.2 "battering":} tires out a defensive target. Parrying (not avoiding or deflecting) attacks from a battering hero costs the defender 1 stamina.
