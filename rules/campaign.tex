...
The players should probably build their own heroes, perhaps starting from scratch with 100xp newbies. Introduce to the game individually with the \texttt{Dungeon of Testing}, then continue as a group with the \texttt{Return of Uchly Namen} campaign, or some adventures, or something else. 

There is already several released campaigns available, and more is on the way. Ping the author if you want the notes from the still unreleased adventures and campaigns. We have hundreds of hours worth of adventures available, spanning several campaigns.
